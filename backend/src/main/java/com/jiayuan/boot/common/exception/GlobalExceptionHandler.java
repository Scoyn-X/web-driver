package com.jiayuan.boot.common.exception;
...
public class GlobalExceptionHandler {
     ...
     * 处理文件太大异常
     * <p>
     * 当上传的文件超过服务器配置的最大限制时，会抛出 MaxUploadSizeExceededException 异常
     */
    @ExceptionHandler(MaxUploadSizeExceededException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public <T> Result<T> processException(MaxUploadSizeExceededException e) {
        log.error(e.getMessage(), e);
        return Result.failed(ResultCode.UPLOAD_FILE_TOO_LARGE, "上传文件超出大小限制");
    }
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public <T> Result<T> handleBizException(BusinessException e, HttpServletRequest request) {
        String code = e.getResultCode() == null ? "-" : e.getResultCode().getCode();
        log.warn("business_exception method={} path={} code={} message={}",
                request.getMethod(), request.getRequestURI(), code, e.getMessage());
